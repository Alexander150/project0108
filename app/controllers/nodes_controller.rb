class NodesController < ApplicationController
	...
	def change
		...
		next_node1 = Node.find(node1.next_node) if node1.next_node != ""
		next_node2 = Node.find(node2.next_node) if node2.next_node != ""
		...
		prev_node1.update(next_node: prev_node1_will.to_s) if !prev_node1.nil?
		prev_node2.update(next_node: prev_node2_will.to_s) if !prev_node2.nil?

		if (next_node1_id != node2.id and next_node2_id != node1.id)
			node1.update(next_node: next_node2_id.to_s)
			node2.update(next_node: next_node1_id.to_s)
		elsif (next_node1_id == node2.id and next_node2_id != node1.id)
			node1.update(next_node: next_node2_id.to_s)
			node2.update(next_node: node1.id.to_s)
		elsif (next_node1_id != node2.id and next_node2_id == node1.id)
			node1.update(next_node: node2.id.to_s)
			node2.update(next_node: next_node1_id.to_s)
		end
